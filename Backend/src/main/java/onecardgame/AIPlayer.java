...
public class AIPlayer implements GameParticipantable {
    ...
    public boolean takeTurn(final GameState gameState,
        ...
        if (accumulatedDraws > MIN_INDEX) {
            ...
            // Must check punishment value: defending card must have >= punishment than
            // attacking card
            ...
            if (attackableCard != null && random.nextInt(PERCENTAGE_BASE) < DEFENSE_PROBABILITY_PERCENT) {
                ...
                try {
                    // Display "AI played" message first, before playCard() displays defense
                    // messages
                    ...
                    if (attackableCard instanceof FaceCard) {
                        handleFaceCardEffect(gameState, gameRule, dealer, isInitialTurn);
                    } else {
            ...
            if (playableCard != null && shouldPlayCard(gameState, gameRule, isInitialTurn)) {
                ...
                tryPlayCard(gameState, gameRule, dealer, isInitialTurn);
    ...
    private void handleAccumulatedDraws(final GameState gameState, final PlayRule gameRule, final Dealer dealer,
            final int accumulatedDraws) {
    ...
    private void tryPlayCard(final GameState gameState,
        ...
        if (multipleCardIndices != null && !multipleCardIndices.isEmpty() &&
            ...
            dropMultipleCards(gameState, gameRule, dealer, multipleCardIndices, isInitialTurn);
            return;
        }
        ...
        try {
            ...
            if (aiCard instanceof NumSevenCard numSeven) {
                // Check AI hand size before removing (to see if there's one card left besides
                // NumSevenCard)
            ...
            // This shouldn't happen since findPlayableCard already validated, but handle
            // gracefully
            // GameMessages.displayFormatted(GameMessages.DEBUG_INVALID_MOVE,
            // e.getMessage());
            return;
        }
    ...
    /**
     * Determines whether the AI should use a NumSevenCard based on probability
     * rules.
     * 
     * @param gameState     The game state
     * @param gameRule      The play rules manager
    ...
    private boolean shouldUseNumSevenCard(final GameState gameState,
        ...
        // Rule 1: If non-AI player has less than 3 cards left, and NumSevenCard is
        // playable,
        ...
        // NEW INDEPENDENT RULE: Strategic shape changing when opponent is close to
        // winning
        ...
        // EXISTING RULES: Original NumSevenCard probability logic (independent from
        // above)
     ...
     * ALL rules use beneficial shape selection (most common shape 80%, second most
     * 20%).
     * This ensures AI always picks a shape that maximizes its chances to play cards
     * next turn.
     * 
     * @param gameState               The game state
    ...
    private String selectRandomShape(final GameState gameState,
        ...
        // Rule 1: If non-AI player has less than 3 cards left, change to beneficial
        // shape
        ...
        // Rule 2: If AI has one card left (besides NumSevenCard), choose same shape
        // (90% chance)
        // Check BEFORE removal: if hand had 2 cards (NumSevenCard + 1 other), then
        // after removal there's 1 left
        ...
        // Rule 3: If opponent has 1-2 cards left and we're using NumSevenCard
        // strategically,
        ...
        // Default: ALL rules should be beneficial - select beneficial shape (not
        // random)
    ...
    /**
     * Strategically selects which matching cards to drop based on lastUsedCard,
     * accumulated draws, and probability.
    ...
    private List<Integer> selectStrategicCards(List<Integer> matchingIndices,
        ...
        // Rule 0: If AI has < 3 cards OR > 10 cards, always prefer attack cards for
        // defense (100%)
        ...
        // Rule 1: If accumulated draws > 6, 100% defend with higher/same tier attack
        // card
        ...
        if (!attackCards.isEmpty() && !normalCards.isEmpty()) {
            if (preferAttackCards) {
                // Prefer attack cards: drop normal first, then attack (attack becomes
                // lastUsedCard)
                selectedIndices.addAll(normalCards);
                selectedIndices.addAll(attackCards);
            } else {
                // Prefer normal cards: drop attack first, then normal (normal becomes
                // lastUsedCard)
                selectedIndices.addAll(attackCards);
                selectedIndices.addAll(normalCards);
            }
     ...
     * Cards are dropped in strategic order: normal cards first, then attack cards
     * last.
    ...
    private void dropMultipleCards(final GameState gameState,
        ...
        // Sort each group in descending order to avoid index shifting when removing
        normalCardIndices.sort((a, b) -> Integer.compare(b, a));
        attackCardIndices.sort((a, b) -> Integer.compare(b, a));

        // Build final play order: normal cards (descending) first, then attack cards
        // (descending) last
        ...
        // Play cards in order: normal cards first (highest index to lowest), then
        // attack cards (highest index to lowest)
        // This ensures attack cards are always played last, and we avoid index shifting
        for (Integer index : playOrder) {
            Card card = aiHand.get(index);
            lastCardPlayed = card; // Track the last card played (will be the last attack card if any, otherwise
                                   // last normal card)
        ...
        if (lastCardPlayed != null) {
            ...
            // If the last card is a FaceCard, AI gets one more turn (handled in
            // handleFaceCardEffect)
            // If the last card is not a FaceCard (but matches previous), turn ends
            if (lastCardPlayed instanceof FaceCard) {
                // Face card was played - AI should play another card
                handleFaceCardEffect(gameState, gameRule, dealer, false);
                return; // handleFaceCardEffect will handle turn ending
            }
        }
    ...
    private void handleFaceCardEffect(final GameState gameState, final PlayRule gameRule, final Dealer dealer,
            final boolean isInitialTurn) {
        // AI played a Face Card. The turn is kept (Dealer handles this).
        // We simply return to allow the Frontend to visualize the move.
        // The Frontend will see it's still AI's turn and call /ai-turn again.
        // On the next call, takeTurn() will be called again, finding the next card to
        // play.
        return;
    }
    ...
    /**
     * Plays a card after a face card was played, bypassing normal matching rules.
     * After a face card, any card is playable (face card effect).
     * Still validates defense rules if under attack.
     */
    private void playCardAfterFaceCard(final GameState gameState,
            final PlayRule gameRule,
            final int cardIndex) throws InvalidMoveException {
        Card cardToPlay = gameState.getAiHand().get(cardIndex);
        Card lastUsedCard = gameState.getLastUsedCard();

        // Still need to validate defense rules if under attack
        int accumulatedDraws = gameRule.getAccumulatedDraws();
        if (accumulatedDraws > 0 && lastUsedCard instanceof Attackable attackingCard) {
            // Under attack - must play a defense card
            if (!(cardToPlay instanceof Attackable defendingCard)) {
                throw new InvalidMoveException(GameMessages.INVALID_MOVE_CANNOT_PROTECT);
            }

            // Check punishment hierarchy
            if (defendingCard.getPunishment() < attackingCard.getPunishment()) {
                throw new InvalidMoveException(GameMessages.INVALID_MOVE_CANNOT_PROTECT);
            }

            // Validate defense card type rules (same shape, etc.)
            if (!gameRule.isDefenseCard(cardToPlay, lastUsedCard)) {
                throw new InvalidMoveException(GameMessages.INVALID_MOVE_CANNOT_PROTECT);
            }
        }

        // After face card, matching rules don't apply - just play the card directly
        // Remove card from hand first
        gameState.getAiHand().remove(cardIndex);
        // Play the card directly (bypasses isCardPlayable validation)
        gameRule.playCard(cardToPlay, false); // false = not initial turn
    }
